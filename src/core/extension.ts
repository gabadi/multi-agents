...
function buildTelegramInstructionsFromQueue(msg: QueuedTelegramUserMessage): string {
  const coordinationGuardrails = isCoordinationIntentTelegram(msg.text)
    ? `\nEXTRA RULES FOR THIS MESSAGE (coordination/delegation detected):\n` +
      `1. Your FIRST output should be a brief plan to the human via Telegram; do not jump directly to editing code locally.\n` +
      `2. If the request involves delegation, creating an agent, changing the plan, rollback, or closure with git/PR, first delegate with a clear contract and maintain correct ownership.\n` +
      `3. If the task already belongs to a sub-coordinator, do not override their worktree from here unless explicit cancellation or reassignment.\n` +
      `4. Commit/push/gh pr create are delegated to the git agent; do not do them ad-hoc from the coordinator.\n\n`
    : "";
...
function formatResponsePayloadForPrompt(payload: Record<string, unknown>): string {
  // Simple text message (from fabric_send_message with payload.text)
  if (typeof payload.text === "string" && payload.text) return payload.text;
  
  // Intermediate message from agent (from fabric_send_message with payload.message)
  if (typeof payload.message === "string" && payload.message) {
    const taskId = String(payload.task_id ?? "unknown");
    return `[MESSAGE] task=${taskId}\n\n${payload.message}`;
  }
  
  // Task completion report (from fabric_report_completion with payload.summary)
  ...
  // Simple status responses (e.g., {status: "accepted", task_id: "..."})
  if (typeof payload.status === "string") {
    const taskId = String(payload.task_id ?? "unknown");
    return `[${payload.status.toUpperCase()}] task=${taskId}`;
  }
...
function isAckLikeMailboxChat(text: string): boolean {
  ...
  if (/\b(blocked|need|can you|please|confirm|approve|review|help|error|fail|urgent|task|contract)\b/i.test(normalized)) {
    return false;
  }

  const ackPatterns = [
    /^(ack|ok|okay|got it|copy that|roger|noted|thanks|thank you)\b/,
    /^(standing by|available)\b/,
    /^no further action required\b/,
    /^(closing thread|closing definitively)\b/,
    /^received and closed\b/,
    /^final ack received\b/,
    /^do not respond( any)? more\b/i,
    /^acknowledged.*closing\b/i,
    /^[👍👌✅]+$/,
  ];
...
function handleMessage(msg: FabricMessage) {
  ...
  switch (msg.type) {
    case "chat": {
      ...
      const replyInstructions = ackLike
        ...
        : `\n\nMAILBOX PROTOCOL\n` +
          ...
          `- to: "${msg.from}"\n` +
          `- type: "chat"\n` +
          `- payload: { "text": "<your compact English reply here>" }\n` +
          `Replace the angle-bracket placeholder with your actual message; never send it verbatim.\n` +
          `Fallback only if the tool is unavailable:\n` +
          `echo '{"message_id":"<random>","from":"${AGENT_ID}","to":"${msg.from}","type":"chat","payload":{"text":"<your compact English reply here>"},"timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> /tmp/fabric-agents/mailboxes/${msg.from}.jsonl && kill -USR1 $(cat /tmp/fabric-agents/pids/${msg.from}.pid)`;
    ...
    case "message": {
      // Intermediate messages from agents (fabric_send_message with payload.message)
      const messageText = String(msg.payload.message ?? msg.payload.text ?? "");
      const taskId = String(msg.payload.task_id ?? "unknown");
      if (!messageText) {
        ctx.ui.notify(`📨 ${msg.from} [message] (empty)`, "info");
        break;
      }
      const replyInstructions = 
        ...
        `- to: "${msg.from}"\n` +
        `- type: "message"\n` +
        `- payload: { "message": "<compact English message>", "task_id": "${taskId}" }`;
      const injected = injectMailboxPrompt(
        `MAILBOX MESSAGE FROM ${msg.from}\n` +
        `task_id: ${taskId}\n\n` +
        `${messageText}${replyInstructions}`,
        "followUp"
      );
      if (!injected) {
        ctx.ui.notify(`💬 ${msg.from}: ${messageText.slice(0, 200)}`, "info");
      }
      break;
    }
    default: {
...
export default function (pi: ExtensionAPI) {
  ...
  if (!ROLE) {
    pi.on("session_start", async (_event, ctx) => {
      ctx.ui.notify("Fabric: FABRIC_ROLE not defined", "warning");
    });
    return;
  }
...
const REPORT_TO = process.env.FABRIC_REPORT_TO || PARENT_AGENT_ID || "";
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        const lines = rows.map(r => {
          ...
          return `• ${r.id} | ${r.version} | ${r.analysis_type} | conf=${r.confidence_score} | kw=[${kw.join(",")}] | ${!r.invalidated ? "✓active" : "✗invalidated"}`;
        });
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      try {
        ...
        const summary = rows.map(r => {
          const kw = JSON.parse(r.keywords || "[]");
          return `[${r.version}] task=${r.task_id} type=${r.analysis_type} conf=${r.confidence_score} kw=${kw.join(",")} ${!r.invalidated ? "✓" : "✗"}`;
        }).join("\n");

        const details = rows.map(r => {
          const prefix = !r.invalidated ? "" : `[INVALIDATED ${r.version}] `;
  ...
  pi.registerTool({
    ...
    description: "Injects the agent context (agent_note) from a task into the current chat. Useful for coordinators to pass context to a worker without writing it manually. Returns the agent_note ready to copy.",
    promptSnippet: "Inject task analysis context directly into the conversation",
    promptGuidelines: [
      "Use pm_inject_task_context to recover agent_note from a task and display it as a text block.",
      "Useful for coordinators to say 'here is the context of what we tried before' to a new worker.",
      "Returns the raw agent_note — ready for the LLM to consume directly.",
      "If no analysis exists for that task, returns empty text with note.",
    ],
    parameters: Type.Object({
      task_id: Type.Number({ description: "Task ID" }),
      extra_context: Type.Optional(Type.String({ description: "Additional context to concatenate" })),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      const context = buildTaskContextForAgent(params.task_id, params.extra_context ?? undefined);
      if (!context) {
        return {
          content: [{ type: "text", text: `(No previous analysis for task ${params.task_id})` }],
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      payload: Type.Union([
        ...
        Type.Object({}, { description: "JSON payload", minProperties: 1, additionalProperties: true }),
      ], { description: "Message payload. For telegram_agent_response use the first variant; for other message types use any non-empty JSON object." }),
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      // Use node directly with tsx loader instead of npx tsx to avoid hanging issues
      // Try to find tsx loader in common locations
      const homeDir = process.env.HOME || "/Users/jescobar";
      const tsxLoaderCandidates = [
        resolve(homeDir, ".npm/_npx/fd45a72a545557e9/node_modules/tsx/dist/loader.mjs"),
        resolve(homeDir, ".local/share/mise/installs/node/22.16.0/lib/node_modules/tsx/dist/loader.mjs"),
        "/opt/homebrew/lib/node_modules/tsx/dist/loader.mjs",
      ];
      let tsxLoader = tsxLoaderCandidates.find((p) => existsSync(p));
      if (!tsxLoader) {
        return { 
          content: [{ type: "text", text: "tsx loader not found. Install tsx globally or ensure npx cache exists." }], 
          details: { error: "tsx_loader_not_found", searched: tsxLoaderCandidates } 
        };
      }
  ...
  // ── Safe Agent Launch (NPOLICY-aware) ──
  pi.registerTool({
    name: "fabric_launch_agent_safe",
    label: "Fabric Launch Agent Safe",
    description: "Launch a new Fabric agent in tmux WITHOUT inheriting NPOLICY network restrictions. Detects if current process has NPOLICY and uses safe launch methods to avoid passing network restrictions to child agents. Essential for agents that need Docker/testcontainers connectivity.",
    promptSnippet: "Launch a new agent avoiding NPOLICY inheritance for Docker/testcontainers compatibility",
    promptGuidelines: [
      "Use fabric_launch_agent_safe when launching agents that need Docker, testcontainers, or network connectivity that might be blocked by NPOLICY.",
      "This tool detects NPOLICY in the current process and automatically uses safe launch methods if present.",
      "The safe launch creates a wrapper that closes file descriptors 3-100 before executing the agent, preventing NPOLICY inheritance.",
      "Use --force-safe to always use safe launch even if NPOLICY not detected.",
      "Returns the pane_id and method used (safe_wrapper, safe_detached, or standard).",
      "All other parameters work the same as fabric_launch_agent."
    ],
    parameters: Type.Object({
      role: Type.String({ description: "Role/skill to load (dev, reviewer, test, etc.)" }),
      agent_id: Type.String({ description: "Unique agent identifier" }),
      mode: Type.Optional(Type.String({ description: "interactive or rpc (default: rpc)" })),
      model: Type.Optional(Type.String({ description: "Override model (e.g. fern/gpt-5.3-codex)" })),
      report_to: Type.String({ description: "Agent ID that receives alive ACK and task reports" }),
      session: Type.Optional(Type.String({ description: "tmux session (default: fabric-default)" })),
      parent_agent_id: Type.Optional(Type.String({ description: "Parent coordinator ID for federated sub-coordinators" })),
      workspace_dir: Type.Optional(Type.String({ description: "Workspace directory/worktree for the agent" })),
      window_name: Type.Optional(Type.String({ description: "Tmux window name (auto-generated if not provided)" })),
      force_safe: Type.Optional(Type.Boolean({ description: "Force safe launch even if NPOLICY not detected" })),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      // Import the safe launcher dynamically
      const { launchAgentSafe } = await import("../tools/launch-agent-safe.js");
      
      const result = await launchAgentSafe({
        role: params.role,
        agent_id: params.agent_id,
        session: params.session || "fabric-default",
        mode: (params.mode as "interactive" | "rpc") || "rpc",
        model: params.model,
        parent_agent_id: params.parent_agent_id,
        workspace_dir: params.workspace_dir,
        report_to: params.report_to,
        window_name: params.window_name,
        force_safe_launch: params.force_safe || false,
        debug: false,
      });

      const text = result.success
        ? `Agent ${result.agent_id} launched successfully.\nMethod: ${result.method}\nSession: ${result.session}\nWindow: ${result.window}${result.pane_id ? `\nPane: ${result.pane_id}` : ""}${result.warning ? `\n⚠️ ${result.warning}` : ""}`
  ...
  // ── Fern Model Management Tools (Simple Inline Version) ──
  const FERN_BASE_URL = "https://fern.addi.com/v1";
  const MODELS_JSON_PATH = `${process.env.HOME || "/Users/jescobar"}/.pi/agent/models.json`;

  const KNOWN_MODELS: Record<string, any> = {
    "gemini-3.1-flash-lite": { name: "Gemini 3.1 Flash Lite", cost: { input: 0.25, output: 1.5 }, contextWindow: 1048576 },
    "minimax-m2.7": { name: "MiniMax M2.7", cost: { input: 0.3, output: 1.2 }, contextWindow: 128000 },
    "qwen3p6-plus": { name: "Qwen 3.6 Plus", cost: { input: 0.5, output: 3.0 }, contextWindow: 128000 },
    "kimi-k2.5": { name: "Kimi K2.5", cost: { input: 0.6, output: 3.0 }, contextWindow: 262144 },
    "kimi-k2.6": { name: "Kimi K2.6", cost: { input: 0.95, output: 4.0 }, contextWindow: 128000 },
    "claude-haiku-4-5": { name: "Claude Haiku 4.5", cost: { input: 1.0, output: 5.0 }, contextWindow: 200000 },
    "glm-5": { name: "GLM-5", cost: { input: 1.0, output: 3.2 }, contextWindow: 128000 },
    "glm-5p1": { name: "GLM-5.1", cost: { input: 1.4, output: 4.4 }, contextWindow: 128000 },
    "gpt-5.3-codex": { name: "GPT-5.3 Codex", cost: { input: 1.75, output: 14.0 }, contextWindow: 272000 },
    "gemini-3.1-pro": { name: "Gemini 3.1 Pro", cost: { input: 2.0, output: 12.0 }, contextWindow: 1048576 },
    "gpt-5.4": { name: "GPT-5.4", cost: { input: 2.5, output: 15.0 }, contextWindow: 1050000 },
    "gpt-5.5": { name: "GPT-5.5", cost: { input: 2.5, output: 15.0 }, contextWindow: 1050000 },
    "claude-sonnet-4-6": { name: "Claude Sonnet 4.6", cost: { input: 3.0, output: 15.0 }, contextWindow: 1000000 },
    "claude-opus-4-6": { name: "Claude Opus 4.6", cost: { input: 5.0, output: 25.0 }, contextWindow: 1000000 },
    "claude-opus-4-7": { name: "Claude Opus 4.7", cost: { input: 5.0, output: 25.0 }, contextWindow: 1000000 },
  };

  pi.registerTool({
    name: "fern_fetch_models",
    label: "Fetch Fern Models",
    description: "Fetch available models from fern proxy and optionally update models.json. Sorted by input cost (lowest first).",
    promptSnippet: "Fetch models from fern API",
    parameters: Type.Object({
      provider: Type.Optional(Type.String({ description: "Provider (default: fern)" })),
      update_json: Type.Optional(Type.Boolean({ description: "Update models.json" })),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      try {
        const apiKey = process.env.LITELLM_API_KEY || "";
        if (!apiKey) {
          return { content: [{ type: "text", text: "Error: LITELLM_API_KEY not set" }], details: { error: "No API key" } };
        }
        ...
        if (!response.ok) {
          return { content: [{ type: "text", text: `API error: ${response.status}` }], details: { error: response.statusText } };
        }
        ...
        const models = (data.data || []).map((m: any) => {
          ...
          return {
            ...
            inputCost: known.cost?.input || 0,
            outputCost: known.cost?.output || 0,
            contextWindow: known.contextWindow || 128000,
          };
        }).sort((a: any, b: any) => a.inputCost - b.inputCost);

        if (params.update_json && existsSync(MODELS_JSON_PATH)) {
          try {
            const existing = JSON.parse(readFileSync(MODELS_JSON_PATH, "utf8"));
            existing.providers = existing.providers || {};
            existing.providers.fern = existing.providers.fern || {};
            existing.providers.fern.models = models.map((m: any) => ({
              id: m.id,
              name: m.name,
              reasoning: false,
              input: ["text"],
            ...
            writeFileSync(MODELS_JSON_PATH, JSON.stringify(existing, null, 2));
          } catch (e) {
            console.error("Failed to update models.json:", e);
          }
        }

        const table = models.map((m: any) => `| ${m.id} | ${m.name} | $${m.inputCost}/M | $${m.outputCost}/M | ${Math.round(m.contextWindow/1000)}k |`).join("\n");
        return {
          content: [{ type: "text", text: `| Model | Name | Input | Output | Context |\n|-------|------|-------|--------|---------|\n${table}` }],
          details: { count: models.length, updated: params.update_json }
        };
      } catch (err: any) {
  ...
  pi.registerTool({
    ...
    description: "Get consumption/quota status from fern proxy API",
    promptSnippet: "Get consumption and quota from fern",
    parameters: Type.Object({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      try {
        ...
        // Call the user/info endpoint (based on check-quota-status_sh.sh)
        const response = await fetch(`${FERN_BASE_URL}/user/info`, {
          headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" }
        });
        ...
        // Calculate spend from keys for this user
        const userId = userInfo.user_id || "unknown";
        const userSpend = keys
          .filter((k: any) => k.user_id === userId)
          .reduce((sum: number, k: any) => sum + (k.spend || 0), 0);

        const userBudget = userInfo.max_budget || 0;
        const userPercent = userBudget > 0 ? (userSpend / userBudget * 100).toFixed(1) : "0";
        const resetDate = userInfo.budget_reset_at ? userInfo.budget_reset_at.split("T")[0] : "N/A";

        // Team info
        const teamSpend = team.spend || 0;
        const teamBudget = team.max_budget || 0;
        const teamPercent = teamBudget > 0 ? (teamSpend / teamBudget * 100).toFixed(1) : "0";
        const teamResetDate = team.budget_reset_at ? team.budget_reset_at.split("T")[0] : "N/A";

        // Build key usage table (filtered by model if specified)
        let userKeys = keys.filter((k: any) => k.user_id === userId);
        if (params.model_id) {
          userKeys = userKeys.filter((k: any) => 
            k.model_aliases?.includes(params.model_id) || 
            k.model_names?.includes(params.model_id)
          );
        }

        const keyRows = userKeys.map((k: any) => {
          const keyName = k.key_alias || k.key_name || "Unnamed";
          const spend = k.spend || 0;
          const models = k.model_aliases?.join(", ") || k.model_names?.join(", ") || "All";
          return `| ${keyName} | $${spend.toFixed(4)} | ${models.substring(0, 30)}${models.length > 30 ? "..." : ""} |`;
        }).join("\n");

        const result = `## 📊 PERSONAL USAGE

| Metric | Value |
|--------|-------|
| Budget | $${userBudget.toFixed(2)} |
| Spent | $${userSpend.toFixed(4)} |
| Usage | ${userPercent}% |
| Reset Date | ${resetDate} |

## 👥 TEAM USAGE (${team.team_alias || "Team"})

| Metric | Value |
|--------|-------|
| Budget | $${teamBudget.toFixed(2)} |
| Spent | $${teamSpend.toFixed(4)} |
| Usage | ${teamPercent}% |
| Reset Date | ${teamResetDate} |

## 🔑 KEYS (${userKeys.length})

| Key | Spend | Models |
|-----|-------|--------|
${keyRows || "| No keys found | - | - |"}
`;
